...
class GrowthEngine {
   ...
   * Encourages shares and invite friends.
   */
  buildViralFooter() {
   ...
   * This makes the bot show up as interactive in private chats.
   */
  ...
  /**
   * Handle incoming Telegram messages (webhook handler logic).
   * Call this from your Express webhook handler.
   */
  ...
  _getAboutMessage() {
    return `*About Deal Bot* 🤖\n\nI'm an automated deal aggregator. I scan the web daily for genuine discounts (20%+ off, min ₹300 savings) and post only verified deals.\n\n_No spam. No ads. Just deals._\n\nFor feedback, reply to any deal post.`;
  }
  ...
  /**
   * Post a "welcome new members" message when the channel grows.
   * Pin a sticky growth message.
   */
  ...
  /**
   * Build sharing text that can be forwarded to WhatsApp groups, other Telegram groups, etc.
   * @param {Object} deal
   * @returns {string}
   */
  ...
  /**
   * Generate a list of Telegram group communities where deal sharing is welcome.
   * These are public groups actively accepting deal shares.
   * The operator should join these groups manually and share 1-2 deals per day.
   */
  getTargetCommunities() {
    return [
      { name: 'Amazon India Deals & Offers', handle: '@AmazonIndiaDeals', type: 'telegram-group' },
      { name: 'Flipkart Offers Group', handle: '@flipkartoffers', type: 'telegram-group' },
  ...
  /**
   * Build a formatted message for sharing in communities (with channel promotion).
   * Human-friendly, not spammy.
   */
