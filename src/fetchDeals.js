/**
   ...
   *   2. DealScraper — auto scrapes Desidime, GrabOn, Coupondunia
   * Then dedupes, history-filters, runs DealIntelligence scoring,
   * attaches affiliate links, and fetches product images.
   */
  ...
  class DealFetcher {
    constructor() {
      ...
      this.fetchImages = (process.env.FETCH_PRODUCT_IMAGES || 'true').toLowerCase() !== 'false';
    }
    ...
    buildSubId(deal) {
      return (deal.id || 'deal') + '-' + Date.now().toString().replace(/[^a-zA-Z0-9-_]/g, '').slice(0, 48);
    }
    ...
    async processDealsWithAffiliateLinks() {
      ...
      for (const deal of this.deals) {
        ...
        try {
          ...
          this.saveAttributionRecord({
            ...
            affiliateLink: deal.affiliateLink, method, subId,
      ...
      // Fetch product images for deals that don't have one yet
      if (this.fetchImages && processedDeals.length > 0) {
        logger.info('Fetching product images...');
        try {
          await batchFetchImages(processedDeals, 4);
        } catch (error) {
          logger.warn('Image batch fetch error', { error: error.message });
        }
      }

      // For Telegram: download local copies of images that were fetched
      for (const deal of processedDeals) {
        if (deal.imageUrl && !deal.localImage) {
          try {
      ...
      logger.info('Processed ' + processedDeals.length + ' deals', {
        ...
        withImages: processedDeals.filter(d => d.imageUrl).length,
